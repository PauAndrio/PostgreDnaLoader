    ...
    for ts in uni.trajectory:
        ...
        if firstSnapshot:
            ...
            for at in uni.atoms:
                ...
                atomBufferMemoryTemp = '%s\t%s\t%s\t%s\t%s\n' % (topologyId, snapshotId, at.position[0], at.position[1], at.position[2]) 
                atomBufferMemory.write(atomBufferMemoryTemp)
            ...
            # FIXME! We would be benefit using COPY STDIN to atom table using stringio buffering
            #cur.execute(atomBuffer)
            cur.copy_expert("COPY atom FROM STDIN", atomBufferMemory)
            ...
            cur.execute(atomBuffer)
            atomBuffer = ''
    #FIXME! Instead commit, here will be placed the copy     
    con.commit()
    cur.close()
